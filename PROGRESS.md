...
| 2026-09-24 | Demo **"Tong" kofe brendiga** moslandi (o'zbekcha): hero shar → shaderda **qahva donagi** (`heroShape`, S-ariqcha), hikoya dona → qo'lda terim → qovurish ("birinchi chirs", qo'l olib tashlandi, chaff bo'laklar) → raqamlar ("Tokchada" kalendar) → Tong → shahar → Espresso/Filtr/Latte; frost doira → **qahva chashkasi**; interfeys tarjima to'plami `lib/ui.ts` + `story.ui`/`story.lang`. Qaror: ranglar/uslub/hikoya mavzuga qarab skill tomonidan tanlanadi (`skill/PLAN.md` → "Mavzuga moslashish qoidasi") |
| 2026-09-24 | **SKILL yozildi**: `skill/story-3d-site/SKILL.md` (7 qadam: brif → hikoya treatment + tasdiq → scaffold → moslash → modellar → QA → ko'rsatish) + `references/` (storytelling, theming, config, scenes-and-effects, models, qa) + `scripts/` (new-site.sh, optimize-glb.sh, blender/run.sh + 3 model skripti, qa/ chapter-shots/walkthrough/gate-test). quick_validate: valid. O'rnatildi: `~/.claude/skills/story-3d-site` → repo'ga symlink (Windows'da: nusxa yoki `mklink /D`) |
| 2026-09-24 | **Skill sinovi (o'zim ishlatdim)**: yangi mavzu — ekologiya kampaniyasi "Ildiz" (o'zbekcha) `skill-tests/eco-ildiz/` (treatment `TREATMENT.md`). SKILL.md qadamlari bo'yicha: scaffold 5 s, config qayta yozildi, hero = **eman yong'og'i** (shaderga `heroShape: 2` qo'shildi), kirish = daraxt kesimi (yillik halqalar), olov bilan `burnThrough`, yashil shahar (`CityGrid parks`). QA: walkthrough (2 gate, konsol toza), gate-test OK. **Topilgan skill xatolari tuzatildi**: (1) gate-test yorug'likni WebGL kanvasdan o'qib doim 0 berardi → endi haqiqiy skrinshotdan; (2) walkthrough finalni inglizcha "Explore" nomidan aniqlardi → tildan mustaqil; (3) birinchi yong'oq "jigarrang shar" bo'lib ko'rindi (qalpoq katta + dumalash) → tik obyektlar faqat tebranadi, qoida SKILL.md'ga yozildi. Yaxshilanishlar shablonga qaytarildi (acorn, upright, parks) |
...
6. [x] `SKILL.md` (ish tartibi: brif → ssenariy → shablondan loyiha → sahnalar → sayqal) + `references/*.md` (storytelling, transitions, effects, interactions, design-system, performance, accessibility).
7. [~] Test promptlar bilan sinov (1-sinov: "Ildiz" ✅) (skill bilan/skillsiz), foydalanuvchi bahosi, iteratsiya.
8. [x] Fedora'da o'rnatildi (symlink). [ ] Windows.
