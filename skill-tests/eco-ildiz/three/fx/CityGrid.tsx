...
export function CityGrid({
  n = 20, gap = 0.37, seed = 7, base = "#8d9c96", accent = "#35d58b", lit = () => 0, parks = 0.16,
}: {
  n?: number; gap?: number; seed?: number; base?: string; accent?: string;
  /** Share of city blocks that are parks (trees), 0..1. */
  parks?: number;
  ...
  const lots = useMemo(() => {
    ...
    for (let x = 0; x < n; x++)
      for (let z = 0; z < n; z++) {
        ...
        const k = rand(s);
        const park = rand(Math.floor(x / 5) * 7 + Math.floor(z / 6) * 13 + seed) > 1 - parks;
