...
// Chapter 3 — the fire (roast). The hero cracks (glowing fissures, set in
// story.config hero keys) — in coffee this is literally the "first crack".
// Flakes of chaff (the bean's silver skin) peel off, orbit it and drift outwards
// while embers rise. (In a "loss" story the same scene reads as a breaking orb.)
...
export default function FractureScene(props: SceneProps) {
  ...
  return (
    <group ref={root}>
      <LightBeams color="#ff5a1f" count={8} spread={8} z={-4} opacity={() => range(p(), 0, 0.25)} />
      <group ref={orbit}>
        {SHARDS.map((s, i) => (
          <group key={i} ref={(el) => { shards.current[i] = el; }} scale={s.s}>
            <GlassShard seed={s.seed} tint="#8a8078" edge="#ff8a50" opacity={0.35} edgeOpacity={0.3} />
          </group>
        ))}
      </group>
      <pointLight position={[0, 2, 3]} intensity={40} color="#ff5a2a" />
      <Particles kind="dot" count={140} color="#ff6a5a" area={[10, 6, 5]} lift={0.08} opacity={() => 0.4 + p() * 0.6} />
      <Particles kind="dot" count={70} color="#ffb060" area={[5, 5, 3]} lift={0.7} sway={0.15} opacity={() => range(p(), 0.5, 0.8)} seed={17} />
    </group>
  );
}
