...
export default function EvidenceScene(props: SceneProps) {
  ...
  const calendar = useMemo(() => makeCalendarTexture({ ...fonts, title: "Yomg‘irsiz", days: ["DU", "SE", "CH", "PA", "JU", "SH", "YA"] }), [fonts]);
  ...
  return (
    <group ref={root}>
      ...
      <LightBeams color="#ff5a1f" count={9} spread={9} z={-6} />
      {stats.map((s, i) => (
        <group key={s.value} ref={(el) => { shards.current[i] = el; }}>
          <group scale={[1.55, 1.2, 1]}>
            <GlassShard seed={PATHS[i % PATHS.length].seed} tint="#ffe9dc" />
          </group>
