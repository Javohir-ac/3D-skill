...
export function Hero() {
  ...
  useFrame((state, dt) => {
    ...
    if (mesh.current) {
      ...
      // shapes with an "up" (acorn) stay upright and only sway; round ones tumble
      const upright = Math.min(1, Math.max(0, c.shape - 1));
      mesh.current.rotation.x += dt * c.spin * 0.35 * (1 - upright);
      mesh.current.rotation.x *= 1 - upright * Math.min(1, dt * 2);
      mesh.current.rotation.z = Math.sin(state.clock.elapsedTime * 0.8) * 0.12 * upright;
    }
    if (halo.current) {
