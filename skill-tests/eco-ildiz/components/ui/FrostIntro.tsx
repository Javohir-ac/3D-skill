...
export function FrostIntro() {
  ...
  return (
    <div ref={layer} className={`frost-intro ${clock ? "is-solved" : ""}`}>
      {clock && (
        <svg
          className="frost-rings"
          aria-hidden="true"
          viewBox="-100 -100 200 200"
          style={{ left: clock.x - clock.r, top: clock.y - clock.r, width: clock.r * 2, height: clock.r * 2 }}
        >
          {/* the drawn circle becomes a tree cross-section: bark, growth rings, a crack */}
          <circle className="bark" r="96" />
          <circle className="ring" r="88" pathLength={1} style={{ animationDelay: `${0.00}s` }} />
          <circle className="ring" r="77" pathLength={1} style={{ animationDelay: `${0.07}s` }} />
          <circle className="ring" r="67" pathLength={1} style={{ animationDelay: `${0.14}s` }} />
          <circle className="ring" r="58" pathLength={1} style={{ animationDelay: `${0.21}s` }} />
          <circle className="ring" r="49" pathLength={1} style={{ animationDelay: `${0.28}s` }} />
          <circle className="ring" r="41" pathLength={1} style={{ animationDelay: `${0.35}s` }} />
          <circle className="ring" r="33" pathLength={1} style={{ animationDelay: `${0.42}s` }} />
          <circle className="ring" r="25" pathLength={1} style={{ animationDelay: `${0.49}s` }} />
          <circle className="ring" r="17" pathLength={1} style={{ animationDelay: `${0.56}s` }} />
          <circle className="ring" r="10" pathLength={1} style={{ animationDelay: `${0.63}s` }} />
          <circle className="pith" r="3.5" />
          <path className="crack" d="M4,-6 L22,-30 L30,-52 L44,-70" pathLength={1} />
        </svg>
      )}
